...
class BlockPool:
    ...
    def __init__(
        ...
        self.enable_hybrid_free_queue = enable_hybrid_free_queue
        ...
        # enabled).
        if not self.enable_hybrid_free_queue:
            self.free_block_queue = FreeKVCacheBlockQueue(self.blocks)
            print("==[COOK]== Using FreeKVCacheBlockQueue.")
        else:
            self.free_block_queue = HybridFreeBlockQueue(self.blocks)
            print("==[COOK]== Using HybridFreeBlockQueue.")
    ...
    def get_new_blocks(self, num_blocks: int, cook_flag: int = 0) -> list[KVCacheBlock]:
        ...
        for block in ret:
            # if cook_flag > 0:
            #     block.offline_ref_count = cook_flag
            #     block.is_offline = True
            # else:
            #     pass
            block.is_offline = cook_flag > 0
            block.offline_ref_count = cook_flag
    ...
    def touch(self, cook_flag: int, blocks: tuple[Sequence[KVCacheBlock], ...]) -> None:
        ...
        # print("block_pool L383: touch", end="")
        # for block in blocks[0]:
        #     print(f"{block.block_id}, {block.is_offline}")
        # for block in self.blocks:
        #     if block.is_offline:
        #         print(f"{block.block_id}", end=",")
        # print(" is the offline blocks."*10)
