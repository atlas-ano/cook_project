...
class LoogleDataset(BenchmarkDataset):
    """
    Implements the LooGLE dataset.  Loads data from a CSV file and generates
    sample requests based on synthetic prompt generation. Only rows with Model
    "GPT-4" and positive response tokens are used.
    """
    ...
    def _sample_loaded_data(self, num_requests: int) -> list:
        ...
        for i in range(len(self.data)):
            artical = self.data[i]["prompt"]
            qa_pairs = self.data[i]["qa_pairs"]
            for qa_pair in qa_pairs:
                if len(filtered_dataset) == num_requests:
                    break
                question = qa_pair["Q"]
                answer = qa_pair["A"]
                prompt = artical + question
                completion = answer
                # if len(prompt) + len(completion) > 40000:
                #     continue
                filtered_dataset.append((prompt, answer))
    ...
    def sample(
        ...
        **kwargs,
        ...
        samples = []
        datasets = self._sample_loaded_data(num_requests=num_requests)
        i = 0
        for prompt, completion in datasets:
            prompt_ids = tokenizer(prompt).input_ids
            completion_ids = tokenizer(completion).input_ids
            prompt_len = len(prompt_ids)
