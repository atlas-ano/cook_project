...
from typing import Any, NewType, TypeAlias, overload, Optional, List
...
class KVCacheBlock:
    ...
    # Mark if this block was ever used by an offline request.
    # Once set to True, never reset to False.
    is_offline: bool = False

    # For offline blocks: expected future reuse count (provided at allocation).
    # Decremented on each cache hit.
    # Only meaningful when is_offline=True.
    offline_ref_count: int = 0

    # Last access timestamp (monotonic counter or actual time)
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f"is_offline={self.is_offline}, offline_ref_count={self.offline_ref_count}, "
...
class FreeKVCacheBlockQueue:
    ...
    def __init__(self, blocks: list[KVCacheBlock]) -> None:
        ...
        # print(f"Free blocks num: {self.num_free_blocks}")
    ...
    def popleft(self) -> KVCacheBlock:
        ...
        if (
            ...
            if self.fake_free_list_head.next_free_block is self.fake_free_list_tail:
                print("fake_head.next_free_block is fake_tail")
            else:
                print("fake_head.next_free_block is None")
...
class HybridFreeBlockQueue:
    """
    A hybrid free block queue that supports:
      - Online blocks: managed by pure LRU (via FreeKVCacheBlockQueue).
      - Offline blocks: managed in a separate queue, evicted based on
        (age / (offline_ref_count + 1)) priority.

    This class wraps two FreeKVCacheBlockQueue instances and provides
    the same core interface as the original, but with smarter eviction.
    """

    def __init__(self, blocks: List[KVCacheBlock], time_fn=None):
        """
        Args:
            blocks: All initial free blocks.
            time_fn: Callable returning current time (for testing). Default: time.monotonic_ns.
        """
        self.time_fn = time_fn or (lambda: time.monotonic_ns())
        self._current_time = 0

        # Split blocks into online/offline (initially all online)
        online_blocks = [b for b in blocks if not b.is_offline]
        offline_blocks = [b for b in blocks if b.is_offline]

        self.online_queue = FreeKVCacheBlockQueue(online_blocks)
        self.offline_queue = FreeKVCacheBlockQueue(offline_blocks)

    def _get_time(self) -> int:
        # Cache time to ensure consistency during one eviction cycle
        if self._current_time == 0:
            self._current_time = self.time_fn()
        return self._current_time

    def popleft(self) -> KVCacheBlock:
        """Evict the block with lowest priority."""
        self._current_time = self.time_fn()  # Refresh time

        # Strategy: Prefer evicting online block (no future value)
        if self.online_queue.num_free_blocks > 0 and self.online_queue.fake_free_list_head.next_free_block != self.online_queue.fake_free_list_tail:
            # print(f"E from onQ. {self.offline_queue.num_free_blocks},{self.online_queue.num_free_blocks}", end=" ")
            # cnt = 0
            # p = self.online_queue.fake_free_list_head
            # while p != self.online_queue.fake_free_list_tail and p is not None:
            #     p = p.next_free_block
            #     cnt += 1
            # cnt_off = 0
            # p = self.offline_queue.fake_free_list_head
            # while p != self.offline_queue.fake_free_list_tail and p is not None:
            #     p = p.next_free_block
            #     cnt_off += 1
            # print(f"{self.num_free_blocks},{cnt_off-1, cnt-1}")
            return self.online_queue.popleft()

        # If no online blocks, choose best candidate from offline
        if self.offline_queue.num_free_blocks <= 0 or self.offline_queue.fake_free_list_head.next_free_block == self.offline_queue.fake_free_list_tail:
            raise ValueError(f"No free blocks available, offline queue={self.offline_queue.num_free_blocks}, online queue={self.online_queue.num_free_blocks}")

        # Linear scan to find block with highest eviction_priority
        # eviction_priority = age / (ref_count + 1)  --> larger = worse = evict first
        candidates = self.offline_queue.get_all_free_blocks()
        if not candidates:
            raise RuntimeError("Offline queue reports blocks but get_all returned empty")

        worst_block = max(
            candidates,
            key=lambda b: (self._current_time - b.last_access_time) / (max((b.offline_ref_count + 1), 1) **3)
        )

        # Remove it from offline queue
        self.offline_queue.remove(worst_block)
        # print(f"E from offQ. {self.offline_queue.num_free_blocks},{self.online_queue.num_free_blocks}", end=" ")
        # cnt = 0
        # p = self.online_queue.fake_free_list_head
        # while p != self.online_queue.fake_free_list_tail and p is not None:
        #     p = p.next_free_block
    ...
    def popleft_n(self, n: int) -> List[KVCacheBlock]:
        """Pop n blocks using hybrid eviction policy."""
        blocks = []
        for _ in range(n):
    ...
    def remove(self, block: KVCacheBlock) -> None:
        """Remove a block from whichever queue it belongs to."""
        
        # print(f"remove block: {block.block_id},offline: {block.is_offline}")
        
        if block.is_offline:
            self.offline_queue.remove(block)
        else:
            self.online_queue.remove(block)

    def append(self, block: KVCacheBlock) -> None:
        """Append block to its corresponding queue."""
        # IMPORTANT: Update last_access_time on free? 
        # No! last_access_time should be set when block is *allocated* or *hit*,
        # not when freed. So we assume it's already up-to-date.
        # print("append block: ", block.block_id)
        # print(block.is_offline)
        if block.is_offline:
            self.offline_queue.append(block)
        else:
            self.online_queue.append(block)
            
        block.last_access_time = self._get_time()
        # block.offline_ref_count = 0
    ...
    def append_n(self, blocks: List[KVCacheBlock]) -> None:
        ...
        for b in blocks:
            if b.is_offline:
                b.last_access_time = self._get_time()
                offline_list.append(b)
            else:
                online_list.append(b)
        if online_list:
            self.online_queue.append_n(online_list)
        if offline_list:
            self.offline_queue.append_n(offline_list)

    @property
    def num_free_blocks(self) -> int:
        return self.online_queue.num_free_blocks + self.offline_queue.num_free_blocks

    # Optional: expose internal queues for debugging
    def get_all_free_blocks(self) -> List[KVCacheBlock]:
        return (
            self.online_queue.get_all_free_blocks() +
            self.offline_queue.get_all_free_blocks()
        )
