...
async def get_request(
    ...
    if trace:
        if "sheet" in trace:
            pass
        elif "burstgpt" in trace.lower():
            import pandas as pd
            df = pd.read_csv(trace)
            column_name = 'Timestamp'
            burstgpt_init_index = 114000 #114000
            print(f'[Cook] Read BurstGPT trace file: {trace}, init_index: {burstgpt_init_index}')
            df_subset = df.iloc[burstgpt_init_index:burstgpt_init_index+total_requests+1]
            df_subset = df_subset.reset_index(drop=True)
            time_series = df_subset[column_name]
            # change series to list:
            delay_ts = time_series.tolist()
            print(delay_ts)
            begin_time = delay_ts[0]
            delay_ts = [s - begin_time for s in delay_ts]
            total_time_actual = delay_ts[-1] - delay_ts[0]
            total_time = total_requests / request_rate
            delay_ts = [delay_ts[i] * total_time / total_time_actual for i in range(total_requests)]
        else:
            raise ValueError(f"Unknown trace file: {trace}")
    ...
    if not trace:
        ...
        print(f"[Cook] delay_ts: {delay_ts}")
...
async def benchmark(
    ...
    # Delete test run for quick benchmark.
...
async def main_async(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # Cook: add cook_flag (int) to extra_body
    extra_body["cook_flag"] = args.cf
